...
export interface StrategyContext {
  ...
  entry: (id: string, direction: 'long' | 'short', qty?: number, stop?: number, limit?: number) => void;
...
export class Interpreter {
          ...
          try {
            ...
            if (payload && payload.action) {
               ...
               if (dir && this.strategy) {
                 ...
                 this.strategy.entry(`webhook_${dir}_${barIndex}`, dir, 1, payload.tp, payload.sl);
  ...
  private evalFunctionCall(node: any, env: Environment): Value {
    ...
    if (node.callee.type === 'MemberExpression') {
      ...
      if (obj === 'ta' && prop === 'sma') {
        ...
        if (prop === 'entry') {
          strat.entry(args[0] as string, args[1] as 'long'|'short', args[2] as number);
          return null;
        } else if (prop === 'close') {
      ...
      if (funcName === 'time') {
        // time used as a function: time(res, session)
        return this.bars[this.currentBarIndex]?.time || 0;
      }
      if (funcName === 'na') {
  ...
  private callPivotHigh(args: any[]): number | null {
     const currHigh = this.bars[this.currentBarIndex]?.high || 0;
     const prevHigh = this.currentBarIndex > 0 ? this.bars[this.currentBarIndex - 1].high : 0;
     if (currHigh > prevHigh * 1.001) return currHigh;
     return null;
  }
  
  private callPivotLow(args: any[]): number | null {
     const currLow = this.bars[this.currentBarIndex]?.low || Infinity;
     const prevLow = this.currentBarIndex > 0 ? this.bars[this.currentBarIndex - 1].low : 0;
     if (currLow < prevLow * 0.999) return currLow;
     return null;
  }
  ...
  private callAtr(args: any[]): number | null {
     ...
     if (prevBar) {
       ...
       if (state.series.length <= this.currentBarIndex) state.series.push(tr);
     }
     const s: number[] = state.series;
     if (s.length < period) return null;
     const atr = s.slice(-period).reduce((a: number, b: number) => a + b, 0) / period;
     return atr;
  }
}
